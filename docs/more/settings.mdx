...
FastMCP does not verify that a proxy is actually configured. If you set `FASTMCP_SSRF_TRUST_PROXY=true` but no `HTTPS_PROXY` / `HTTP_PROXY` is present in the server process's environment (or `NO_PROXY` excludes the target), httpx makes a **direct, unpinned request with the IP blocklist disabled** — there is no fallback to the default protection. Enable this setting only together with an active proxy.
</Warning>
